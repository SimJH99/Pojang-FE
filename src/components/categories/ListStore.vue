<template>
  <div class="w-2/3 mx-auto grid grid-cols-2 mb-3">
    <router-link v-for="store in filteredStores" :key="store.id" :to="getStoreDetailsLink(store.id)" :class="{
      'p-4 border border-gray-300 rounded-md flex items-center mx-2 my-1': store.status === 'OPEN',
      'p-4 border border-gray-300 bg-gray-100 rounded-md flex items-center mx-2 my-1': store.status === 'CLOSED',
    }">
      <img :src="store.imageUrl" :class="{
      'w-[70px] h-[70px] bg-gray-100 object-cover mb-2 rounded-md mr-4': store.status === 'OPEN',
...
// 페이지 관련 변수들을 초기화한다.
watchEffect(() => {
  props.searchName,props.category
  stores.value = [];
  currentPage = 0;
  isLastPage = false;
  fetchData();
...
async function fetchData() {
  ...
  try {
    ...
    const responseAddress = await axios.get(`${process.env.VUE_APP_API_BASE_URL}/api/members/my-info`, { headers });
    const address = responseAddress.data.result;
    const params = {
